...
export function handleStreamError(err: unknown): ApiError {
  ...
  // Extract error details from the AI SDK error for logging purposes
  ...
  if (aiError) {
    ...
    // Error details processed for user-friendly response

    // Return generic error messages to users regardless of specific error type
    if (aiError.statusCode === 402) {
      // Payment required - keep specific message as it's user-actionable
      const message = "Insufficient credits or payment required"
      return Object.assign(new Error(message), {
        statusCode: 402,
        code: "PAYMENT_REQUIRED",
      })
    } else if (aiError.statusCode === 401) {
      // Authentication error - keep specific message as it's user-actionable
      const message = "Invalid API key or authentication failed. Please check your API key in settings."
      return Object.assign(new Error(message), {
        statusCode: 401,
        code: "AUTHENTICATION_ERROR",
      })
    } else if (aiError.statusCode === 429) {
      // Rate limit - keep specific message as it's user-actionable
      ...
      // Other client errors - return generic message
      ...
      // Server errors or other issues - return generic message
      const message = "Server error occurred"
      return Object.assign(new Error(message), {
...
export function extractErrorMessage(error: unknown): string {
  ...
  if (error instanceof Error) {
    ...
    // Check for specific error patterns that should be shown to users
    if (
  ...
  if (aiError) {
    ...
    if (aiError.statusCode === 401) {
      ...
      try {
        JSON.parse(aiError.responseBody)
        // Response body processed for error handling
      } catch {
...
export function createErrorResponse(error: {
  ...
  // Error details processed for response creation
  
  // Handle daily limit first (existing logic) - keep specific message as it's user-actionable
  ...
  // Handle authentication and payment errors with specific messages as they're user-actionable
