                  ...
                  <div className="flex flex-col items-center justify-center py-8 text-center">
                    ...
                    <AnimatePresence>
                      <motion.div 
                        ...
                        transition={{ 
                          duration: 0.75,
                          ease: [0.23, 1, 0.32, 1]  // Smoother ease-out-quint curve
                        }}
                      >
                        {currentTaskTools.map((invocation) => {
                        // For browser tools when not showing additional info, render screenshot directly
                        if (invocation.toolName.toLowerCase().startsWith('browser') && !showAdditionalInfo) {
                          // Extract screenshot
                          let screenshot = invocation.frontendScreenshot
                          if (!screenshot && invocation.result && typeof invocation.result === 'object') {
                            // Parse result if needed
                            let parsedResult = invocation.result
                            if ('content' in invocation.result) {
                              if (Array.isArray(invocation.result.content)) {
                                const textContent = invocation.result.content.find((item: any) => item.type === 'text')
                                if (textContent?.text) {
                                  try {
                                    parsedResult = JSON.parse(textContent.text)
                                  } catch {
                              ...
                              } else if (typeof invocation.result.content === 'string') {
                                try {
                                  parsedResult = JSON.parse(invocation.result.content)
                                } catch {
                                  parsedResult = invocation.result.content
                                }
                              }
                            }
                            if (parsedResult && typeof parsedResult === 'object') {
                              screenshot = parsedResult.screenshot || parsedResult.frontendScreenshot
                            }
                          }
                          
                          // Update last screenshot when we have a new one
                          if (screenshot && screenshot !== lastBrowserScreenshot) {
                            // Use a timeout to avoid immediate state updates during render
                            setTimeout(() => {
                              setLastBrowserScreenshot(screenshot)
                            }, 50)
                          }
                          
                          // Always use screenshot if available, or fallback to last one
                          ...
                          if (displayScreenshot) {
                            return (
                              ...
                              >
                                <div className="relative rounded-lg overflow-hidden border border-border/50 bg-background">
                                  {/* Smooth crossfade between screenshots */}
                                  <AnimatePresence>
                                    <motion.img
                                      key={displayScreenshot}
                                  ...
                                  {/* Show loading overlay when processing without screenshot */}
                                  {!screenshot && lastBrowserScreenshot && (
                                    <motion.div 
                                      className="absolute inset-0 flex items-center justify-center bg-gradient-to-b from-black/20 via-black/40 to-black/20"
                                      initial={{ opacity: 0 }}
                                      animate={{ opacity: 1 }}
                                    ...
                                    >
                                      <div className="relative">
                                        ...
                                        {/* Main badge */}
                                        <div className="relative bg-black/90 backdrop-blur-md px-4 py-2 rounded-full border border-white/10">
                                          <div className="flex items-center gap-2">
                                            {/* Animated dots */}
                                            ...
                                            <span className="text-[11px] text-white/80 font-light tracking-wider uppercase">
                                              Processing
                                            </span>
                                          </div>
                                        </div>
                            ...
                            // No screenshot available - show last screenshot with overlay or placeholder
                                ...
                                >
                                  <div className="relative rounded-lg overflow-hidden border border-border/50 bg-black">
                                    <motion.img
                                      ...
                                      alt="Browser Screenshot (Previous)"
                                      className="w-full h-auto"
                                      loading="lazy"
                                      style={{ maxHeight: '75vh', objectFit: 'contain', filter: 'brightness(0.7)' }}
                                    />
                                    <motion.div 
                                      className="absolute inset-0 flex items-center justify-center bg-gradient-to-b from-black/20 via-black/40 to-black/20"
                                      initial={{ opacity: 0 }}
                                      ...
                                      <div className="relative">
                                        ...
                                        <div className="relative bg-black/90 backdrop-blur-md px-4 py-2 rounded-full border border-white/10">
                                          <div className="flex items-center gap-2">
                                            ...
                                            >
                                              <motion.div
                                                className="w-full h-0.5 bg-white/40 mt-0.5"
                            ...
                            return (
                              <motion.div
                                ...
                                className="w-full"
                              >
                                <div className="relative rounded-lg overflow-hidden border border-zinc-800/30 bg-gradient-to-b from-black/60 to-black/80" style={{ minHeight: '600px', maxHeight: '80vh' }}>
                                  <div className="flex items-center justify-center h-full">
                                    <div className="relative">
                                      {/* Subtle pulse effect */}
                                      <motion.div
                                        className="absolute -inset-8 bg-white/5 rounded-full blur-2xl"
                                      ...
                                      <div className="relative text-center">
                                        ...
                                        <div className="relative inline-block mb-4">
                                          ...
                                          >
                                            <Desktop className="w-10 h-10 text-zinc-600/50" />
                                          </motion.div>
                                          
                                          {/* Scanning line effect */}
                                          <motion.div
                                            className="absolute inset-x-0 h-px bg-gradient-to-r from-transparent via-zinc-400/50 to-transparent"
                                            animate={{ top: ["0%", "100%", "0%"] }}
                                        ...
                                        <div className="flex items-center justify-center gap-2">
                                          <span className="text-[11px] text-zinc-500/80 font-light tracking-wider uppercase">
                                            Initializing
                                          </span>
                        ...
                        // For directory_list tool when not showing additional info, render directory view directly
                        if (invocation.toolName === 'directory_list' && !showAdditionalInfo) {
                          // Show loading state if still processing
                          if ((invocation.state === 'call' || invocation.state === 'partial-call') && !invocation.result) {
                            return (
                              <motion.div
                                key={invocation.id}
                                initial={{ opacity: 0, filter: "blur(10px)" }}
                              ...
                              >
                                <div className="relative rounded-xl overflow-hidden bg-black border border-zinc-800" style={{ minHeight: '600px', maxHeight: '80vh', display: 'flex', flexDirection: 'column' }}>
                                  ...
                                  <div className="p-4 font-mono text-[13px] bg-black overflow-auto flex-1">
                                    <div className="flex items-start gap-2">
                                      <span className="text-zinc-500 select-none animate-pulse">›</span>
                                      <span className="text-zinc-400 font-medium">listing directory...</span>
                                    </div>
                                  </div>
                                </div>
                              </motion.div>
                          ...
                          if (parsedResult && typeof parsedResult === 'object') {
                            const { success, dirpath, items, summary, error } = parsedResult as any
                                  ...
                                  <div className="relative rounded-xl overflow-hidden bg-black border border-zinc-800" style={{ minHeight: '600px', maxHeight: '80vh', display: 'flex', flexDirection: 'column' }}>
                                    ...
                                    <div className="p-4 font-mono text-[13px] overflow-auto bg-black flex-1">
                                      ...
                                      <div className="space-y-1">
                                        {/* Show directories first */}
                                        {directories.map((item: any, index: number) => (
                                          <div key={`dir-${index}`} className="flex items-center gap-3 text-zinc-400">
                                            <span className="text-blue-400 text-[11px]">[Dir]</span>
                                            <span className="flex-1">{item.name}/</span>
                                            {item.item_count !== undefined && (
                                              <span className="text-zinc-600 text-[11px]">({item.item_count} items)</span>
                                        ...
                                        {/* Show files */}
                                        {files.map((item: any, index: number) => (
                                          <div key={`file-${index}`} className="flex items-center gap-3 text-zinc-400">
                                            <span className="text-green-400 text-[11px]">[File]</span>
                                            <span className="flex-1">{item.name}</span>
                                            {item.size !== undefined && (
                                              <span className="text-zinc-600 text-[11px]">
                        ...
                        // For terminal tools when not showing additional info, render terminal view directly
                        if (invocation.toolName.toLowerCase().startsWith('terminal') && !showAdditionalInfo) {
                          // Show loading state if still processing
                          if ((invocation.state === 'call' || invocation.state === 'partial-call') && !invocation.result) {
                            const operation = invocation.toolName === 'terminal_execute' ? 'Executing command' :
                                           invocation.toolName === 'terminal_read' ? 'Reading output' :
                                           invocation.toolName === 'terminal_connect' ? 'Connecting' :
                                           invocation.toolName === 'terminal_clear' ? 'Clearing screen' :
                                           invocation.toolName === 'terminal_close' ? 'Closing session' : 'Processing'
                                ...
                                <div className="relative rounded-xl overflow-hidden bg-black border border-zinc-800" style={{ minHeight: '600px', maxHeight: '80vh', display: 'flex', flexDirection: 'column' }}>
                                  ...
                                  <div className="flex items-center justify-between px-4 py-2.5 bg-black border-b border-zinc-800">
                                    <div className="flex items-center gap-3">
                                      ...
                                      <div className="h-3 w-px bg-zinc-800" />
                                      <span className="text-[11px] text-zinc-500 font-mono tracking-wider uppercase">Terminal</span>
                                    </div>
                                  </div>
                                  
                                  {/* Terminal-style Loading Body */}
                                  <div className="p-4 font-mono text-[13px] bg-black overflow-auto flex-1">
                                    <div className="flex items-start gap-2">
                                      <span className="text-zinc-500 select-none animate-pulse">â€º</span>
                                      <div>
                                        <span className="text-zinc-400 font-medium">{operation.toLowerCase()}...</span>
                          ...
                          if (parsedResult && typeof parsedResult === 'object') {
                            const { command, output, error, exit_code, stdout, stderr, last_command, window_id } = parsedResult as any
                            const terminalOutput = output || stdout || ''
                            const terminalError = error || stderr || ''
                            const executedCommand = command || last_command || ''
                            
                            // Update last terminal output when we have new content
                            if ((executedCommand || terminalOutput || terminalError) && 
                                (executedCommand !== lastTerminalOutput?.command || 
                                 terminalOutput !== lastTerminalOutput?.output ||
                                 terminalError !== lastTerminalOutput?.error)) {
                              setTimeout(() => {
                            ...
                            // For terminal_execute or terminal_read - show full terminal with smooth transitions
                            if (invocation.toolName === 'terminal_execute' || invocation.toolName === 'terminal_read') {
                              // Use current terminal output or fall back to last one for smooth transitions
                              const displayCommand = executedCommand || lastTerminalOutput?.command || ''
                              const displayOutput = terminalOutput || lastTerminalOutput?.output || ''
                              const displayError = terminalError || lastTerminalOutput?.error || ''
                              const hasContent = displayCommand || displayOutput || displayError
                                    ...
                                    <div className="relative rounded-xl overflow-hidden bg-black border border-zinc-800" style={{ minHeight: '600px', maxHeight: '80vh', display: 'flex', flexDirection: 'column' }}>
                                      <div className="flex items-center justify-between px-4 py-2.5 bg-black border-b border-zinc-800">
                                        ...
                                        {exit_code !== undefined && (
                                          <span className={cn(
                                            "text-[10px] font-mono tracking-wide",
                                            exit_code === 0 ? "text-zinc-500" : "text-zinc-400"
                                          )}>
                                            {exit_code === 0 ? 'âœ“' : `âœ— ${exit_code}`}
                                          </span>
                                        )}
                                      </div>
                                      <div className="p-4 font-mono text-[13px] overflow-auto bg-black flex-1 relative">
                                        <AnimatePresence>
                                          <motion.div
                                            key={`${displayCommand}-${displayOutput?.substring(0, 50)}-${displayError?.substring(0, 50)}`}
                                            initial={{ opacity: 0, filter: 'blur(2px)' }}
                                            animate={{ opacity: 1, filter: 'blur(0px)' }}
                                            exit={{ opacity: 0, filter: 'blur(2px)', position: 'absolute', top: 0, left: 0, right: 0, padding: '1rem' }}
                                            transition={{ duration: 0.4, ease: [0.25, 0.1, 0.25, 1] }}
                                          >
                                            {displayCommand && (
                                              <div className="mb-3 flex items-start gap-2">
                                                <span className="text-zinc-500 select-none">â€º</span>
                                                <span className="text-zinc-100 font-medium">{displayCommand}</span>
                                              </div>
                                            )}
                                            {displayOutput && (
                                              <pre className="text-zinc-400 whitespace-pre-wrap break-all leading-[1.6] font-light">
                                                {(() => {
                                                  ...
                                                  return (
                                                    <>
                                                      {displayLines.join('\n')}
                                                      {truncated && (
                                                        <span className="text-zinc-600 italic block mt-2">
                                                          ... {lines.length - maxLines} more lines (truncated)
                                                        </span>
                                                      )}
                                                    </>
                                            ...
                                            {displayError && (
                                              <pre className="text-zinc-300 whitespace-pre-wrap break-all mt-2 leading-[1.6] opacity-90">
                                                {(() => {
                                        ...
                                        {/* Show processing overlay when using last output */}
                                        {!terminalOutput && !terminalError && !executedCommand && lastTerminalOutput && (
                                          <motion.div 
                                            className="absolute inset-0 flex items-center justify-center bg-gradient-to-b from-black/10 via-black/30 to-black/10"
                                            initial={{ opacity: 0 }}
                                            animate={{ opacity: 1 }}
                                            transition={{ duration: 0.3, ease: "easeOut" }}
                                          >
                                            <div className="relative">
                                              {/* Subtle glow */}
                                              <div className="absolute inset-0 blur-xl bg-zinc-500/5 rounded-full" />
                                              
                                              {/* Terminal-style badge */}
                                              <div className="relative bg-zinc-900/95 backdrop-blur-md px-4 py-2 rounded-full border border-zinc-800/50">
                                                <div className="flex items-center gap-2">
                                                  {/* Terminal cursor animation */}
                                                  <motion.div
                                                    className="w-2 h-3 bg-zinc-400/70"
                                                    animate={{ opacity: [1, 0, 1] }}
                                                    transition={{ duration: 1, repeat: Infinity }}
                                                  />
                                                  
                                                  <span className="text-[11px] text-zinc-400/90 font-mono tracking-wider">
                                                    PROCESSING
                                                  </span>
                                                </div>
                                              </div>
                            ...
                            // For terminal_close, don't show anything
                            if (invocation.toolName === 'terminal_close') {
                              return null
                            }
                            
                            // For other terminal commands, show minimal status with crossfade
                            return (
                              <motion.div
                                key={invocation.id}
                                initial={{ opacity: 0, filter: "blur(10px)" }}
                                ...
                                className="text-xs text-muted-foreground italic p-4 text-center"
                              >
                                {invocation.toolName === 'terminal_connect' ? 'Terminal session connected' :
                                 invocation.toolName === 'terminal_clear' ? 'Terminal screen cleared' :
                        ...
                        // For file tools when not showing additional info
                        if (invocation.toolName.toLowerCase().startsWith('file_') && !showAdditionalInfo) {
                          // Show loading state if still processing
                          if ((invocation.state === 'call' || invocation.state === 'partial-call') && !invocation.result) {
                            const operation = invocation.toolName === 'file_read' ? 'Reading' :
                                           invocation.toolName === 'file_write' ? 'Writing' :
                                           invocation.toolName === 'file_edit' ? 'Editing' :
                                           invocation.toolName === 'file_delete' ? 'Deleting' :
                                           invocation.toolName === 'file_exists' ? 'Checking' :
                                           invocation.toolName === 'file_append' ? 'Appending' : 'Processing'
                                ...
                                <div className="relative rounded-xl overflow-hidden bg-black border border-zinc-800" style={{ minHeight: '600px', maxHeight: '80vh', display: 'flex', flexDirection: 'column' }}>
                                  ...
                                  <div className="p-4 font-mono text-[13px] bg-black overflow-auto flex-1">
                                    <div className="flex items-start gap-2">
                                      <span className="text-zinc-500 select-none animate-pulse">â€º</span>
                                      <div>
                                        <span className="text-zinc-400 font-medium">{operation.toLowerCase()} file...</span>
                                        {invocation.args?.filepath && (
                                          <div className="text-zinc-600 text-[11px] mt-1">
                                            {invocation.args.filepath}
                                          </div>
                                        )}
                        ...
                        // Old file handling code kept for reference but not used
                        if (false && invocation.toolName.toLowerCase().startsWith('file_') && !showAdditionalInfo) {
                          // Parse result
                          let parsedResult = invocation.result
                          if (invocation.result && typeof invocation.result === 'object') {
                            if ('content' in invocation.result) {
                          ...
                          if (parsedResult && typeof parsedResult === 'object') {
                            const { success, filepath, content, exists, is_file, is_directory, size, error, message } = parsedResult as any
                            
                            // For file_read - show consistent terminal-style file viewer
                            if (invocation.toolName === 'file_read' && (content || error || filepath)) {
                              const lines = content ? content.split('\n') : []
                              const maxLines = 30  // Limit display lines for performance
                              const truncated = lines.length > maxLines
                              const displayLines = truncated ? lines.slice(0, maxLines) : lines
                                  ...
                                  <div className="relative rounded-xl overflow-hidden bg-black border border-zinc-800" style={{ minHeight: '600px', maxHeight: '80vh', display: 'flex', flexDirection: 'column' }}>
                                    ...
                                    <div className="p-4 font-mono text-[13px] overflow-auto bg-black flex-1">
                                      ...
                                      {filepath && (
                                        <div className="mb-3 flex items-start gap-2">
                                          <span className="text-zinc-500 select-none">›</span>
                                          <span className="text-zinc-100 font-medium">cat {filepath}</span>
                                        </div>
                                      )}
                                      ...
                                      {content ? (
                                        <>
                                          <pre className="text-zinc-400 whitespace-pre-wrap leading-relaxed font-light">
                                            {displayLines.join('\n')}
                                          </pre>
                                          {truncated && (
                                            <div className="text-zinc-600 italic text-[11px] mt-3 pt-3 border-t border-zinc-800">
                            ...
                            // For other file operations, show git-style status
                            const operation = invocation.toolName === 'file_write' ? 'create' :
                                            invocation.toolName === 'file_edit' ? 'modify' :
                                            invocation.toolName === 'file_append' ? 'append' :
                                            invocation.toolName === 'file_delete' ? 'remove' :
                                            invocation.toolName === 'file_exists' ? 'stat' : 'file'
                              ...
                              >
                                <div className="relative rounded-lg overflow-hidden bg-gray-950 border border-gray-800">
                                  <div className="px-4 py-3 font-mono text-xs space-y-1.5">
                                    <div className="flex items-center gap-2">
                                      <span className={
                                        invocation.toolName === 'file_delete' ? (success ? 'text-red-500' : 'text-gray-500') :
                                        invocation.toolName === 'file_write' ? (success ? 'text-green-500' : 'text-red-500') :
                                        invocation.toolName === 'file_exists' ? (exists ? 'text-green-500' : 'text-gray-600') :
                                        success ? 'text-green-500' : 'text-red-500'
                                      }>
                                        {invocation.toolName === 'file_delete' ? (success ? '-' : '!') :
                                         invocation.toolName === 'file_write' ? (success ? '+' : '!') :
                                         invocation.toolName === 'file_exists' ? (exists ? 'âœ“' : '?') :
                                         success ? '~' : '!'}
                                      </span>
                                      <span className="text-gray-400">{operation}</span>
                                      <span className={
                                        invocation.toolName === 'file_delete' && success ? 'text-red-400 line-through' :
                                        invocation.toolName === 'file_exists' ? (exists ? 'text-blue-400' : 'text-gray-500') :
                                        'text-blue-400'
                                      }>
                                        {filepath}
                                      </span>
                                    </div>
                                    {success && invocation.toolName !== 'file_exists' && invocation.toolName !== 'file_delete' && (
                                      <div className="pl-4 text-gray-500 text-[11px]">
                                        {invocation.toolName === 'file_write' && '1 file created'}
                                        {invocation.toolName === 'file_edit' && '1 file modified'}
                                        {invocation.toolName === 'file_append' && '1 file updated'}
                                      </div>
                                    )}
                                    {invocation.toolName === 'file_exists' && exists && (
                                      <div className="pl-4 flex items-center gap-3 text-[11px] text-gray-500">
                                        {is_file && <span>â€¢ regular file</span>}
                                        {is_directory && <span>â€¢ directory</span>}
                                        {size !== undefined && <span>â€¢ {size} bytes</span>}
                                      </div>
                                    )}
                                    {invocation.toolName === 'file_exists' && !exists && (
                                      <div className="pl-4 text-gray-600 text-[11px]">
                                        No such file or directory
                                      </div>
                                    )}
                                    {error && (
                                      <div className="pl-4 text-red-400 text-[11px]">
                                        error: {error}
                                      </div>
                                    )}
                                  </div>
                        ...
                        // For non-visual tools in progress, show last terminal or screenshot if available
                        if (!showAdditionalInfo && (invocation.state === 'call' || invocation.state === 'partial-call') && !invocation.result) {
                          // Prefer last terminal output over screenshot for non-visual tools
                          if (lastTerminalOutput && (lastTerminalOutput.command || lastTerminalOutput.output || lastTerminalOutput.error)) {
                            return (
                              <motion.div
                                key={invocation.id}
                                initial={{ opacity: 0, filter: "blur(10px)" }}
                                ...
                                <div className="relative rounded-xl overflow-hidden bg-black border border-zinc-800" style={{ minHeight: '600px', maxHeight: '80vh', display: 'flex', flexDirection: 'column' }}>
                                  <div className="flex items-center justify-between px-4 py-2.5 bg-black border-b border-zinc-800">
                                    <div className="flex items-center gap-3">
                                      ...
                                      <div className="h-3 w-px bg-zinc-800" />
                                      <span className="text-[11px] text-zinc-500 font-mono tracking-wider uppercase">Terminal (Previous)</span>
                                    </div>
                                  </div>
                                  <div className="p-4 font-mono text-[13px] bg-black overflow-auto flex-1 relative">
                                    <motion.div
                                      initial={{ opacity: 0.7 }}
                                      animate={{ opacity: 0.7 }}
                                      style={{ filter: 'brightness(0.8)' }}
                                    >
                                      {lastTerminalOutput.command && (
                                        <div className="mb-3 flex items-start gap-2">
                                          <span className="text-zinc-500 select-none">â€º</span>
                                          <span className="text-zinc-100 font-medium">{lastTerminalOutput.command}</span>
                                        </div>
                                      )}
                                    ...
                                    >
                                      <div className="text-xs text-zinc-400/90 bg-black/70 px-3 py-1.5 rounded-full backdrop-blur-sm">
                                        {getToolDisplayName(invocation.toolName, invocation.state)}
                                      </div>
                                    </motion.div>
                                  </div>
                                </div>
                          ...
                          } else if (lastBrowserScreenshot) {
                            // Fallback to last screenshot if no terminal output
                            return (
                              <motion.div
                                key={invocation.id}
                                initial={{ opacity: 0, filter: 'blur(6px)' }}
                              ...
                              >
                                <div className="relative rounded-lg overflow-hidden border border-border/50 bg-black">
                                  <motion.img
                                    ...
                                    className="w-full h-auto"
                                    loading="lazy"
                                    style={{ maxHeight: '400px', objectFit: 'contain', filter: 'brightness(0.7)' }}
                                  />
                                  <motion.div 
                                    className="absolute inset-0 flex items-center justify-center bg-black/30"
                                    initial={{ opacity: 0 }}
                                  ...
                                  >
                                    <div className="text-xs text-white/90 bg-black/70 px-3 py-1.5 rounded-full backdrop-blur-sm">
                                      {getToolDisplayName(invocation.toolName, invocation.state)}
                                    </div>
                                  </motion.div>
                                </div>
                        ...
                        // Skip rendering collapsible cards for web/image search - they have their own special display
                        if (invocation.toolName === 'webSearch' || invocation.toolName === 'googleSearch' || invocation.toolName === 'imageSearch') {
                          // Parse result to show the special rendering
                          let parsedResult = invocation.result
                          ...
                          if ((invocation.state === 'call' || invocation.state === 'partial-call') && !invocation.result) {
                            return (
                              <motion.div
                                ...
                                className="w-full"
                              >
                                <div className="relative rounded-2xl overflow-hidden bg-gradient-to-b from-zinc-900/95 to-black/95 backdrop-blur-xl border border-zinc-800/50" style={{ minHeight: '500px' }}>
                                  {/* Modern Header matching search results */}
                                  <div className="flex items-center justify-between px-5 py-3.5 bg-gradient-to-r from-zinc-900/50 via-zinc-800/30 to-zinc-900/50 border-b border-zinc-800/30">
                                    <div className="flex items-center gap-3">
                                      <div className="relative">
                                        <div className="absolute inset-0 bg-gradient-to-r from-blue-500/20 to-violet-500/20 blur-xl" />
                                        {invocation.toolName === 'imageSearch' ? (
                                          <Image className="relative h-4 w-4 text-zinc-400" />
                                        ) : (
                                          <MagnifyingGlass className="relative h-4 w-4 text-zinc-400" />
                                        )}
                                      </div>
                                      <span className="text-sm font-medium text-zinc-300">
                                        {invocation.toolName === 'imageSearch' ? 'Image Search' : 'Search Results'}
                                      </span>
                                    </div>
                                    <div className="flex items-center gap-2">
                                      <div className="px-2.5 py-1 rounded-full bg-zinc-800/50 backdrop-blur-sm">
                                  ...
                                  {/* Loading Body */}
                                  <div className="flex items-center justify-center h-full bg-gradient-to-b from-transparent to-black/20">
                                    <div className="text-center">
                                      {/* Animated search icon - subtle pulse only */}
                                      <div className="relative inline-block mb-4">
                                        <motion.div
                                          animate={{ 
                                            scale: [1, 1.1, 1]
                                          ...
                                          className="relative"
                                        >
                                          <div className="absolute inset-0 bg-gradient-to-r from-blue-500/30 to-violet-500/30 blur-2xl" />
                                          <div className="relative w-12 h-12 rounded-full bg-gradient-to-br from-zinc-800/50 to-zinc-900/50 border border-zinc-700/50 flex items-center justify-center">
                                            {invocation.toolName === 'imageSearch' ? (
                                      ...
                                      <div className="flex items-center justify-center">
                                        <span className="text-sm text-zinc-400">
                                          {invocation.toolName === 'imageSearch' ? 'Searching images' : 'Searching web'}
                                        </span>
                                      </div>
                                    </div>
                                  </div>
                        ...
                        // Regular rendering for non-browser/non-terminal/non-file/non-search tools or when showing additional info
                        return (
                      <motion.div
                        key={invocation.id}
                        ...
                        className={cn(
                          ...
                          invocation.state === 'call' || invocation.state === 'partial-call' 
                            ? "bg-orange-50/50 dark:bg-orange-950/20 border border-orange-200 dark:border-orange-800/50 shadow-sm"
                            : "bg-muted/30 border border-border/50 shadow-sm hover:shadow-md hover:border-border/80",
                          newInvocationIds.has(invocation.id) && "ring-2 ring-green-500/50 shadow-green-500/10"
                        )}
                      >
                        <button
                          onClick={() => toggleToolExpansion(invocation.id)}
                          className="w-full p-3 hover:bg-muted/50 transition-colors"
                        >
                          <div className="flex flex-col gap-2">
                            <div className="flex items-center justify-between">
                              <div className="flex items-center gap-2">
                                {getToolIcon(invocation.toolName, invocation)}
                              ...
                              <div className="flex items-center gap-2">
                                {invocation.state === 'call' ? (
                                  <div className="inline-flex items-center rounded-full bg-orange-100 dark:bg-orange-900/30 px-2 py-0.5 text-xs text-orange-700 dark:text-orange-400">
                                    <Spinner className="mr-1 h-3 w-3 animate-spin" />
                                    Starting...
                                  </div>
                                ) : invocation.state === 'partial-call' ? (
                                  <div className="inline-flex items-center rounded-full bg-blue-100 dark:bg-blue-900/30 px-2 py-0.5 text-xs text-blue-700 dark:text-blue-400">
                                    <Spinner className="mr-1 h-3 w-3 animate-spin" />
                                    In Progress...
                                  </div>
                                ) : invocation.state === 'result' ? (
                                ...
                                <CaretRight 
                                  className={cn(
                                    ...
                                    expandedTools.has(invocation.id) && "rotate-90"
                                  )}
                                />
                              </div>
                            </div>
                            {/* Quick preview when collapsed */}
                            {!expandedTools.has(invocation.id) && invocation.state === 'result' && (
                                ...
                                {(() => {
                                  ...
                                  if (invocation.result && typeof invocation.result === 'object' && 'content' in invocation.result) {
                                    ...
                                    if (Array.isArray(invocation.result.content)) {
                                      ...
                                      // If content is an object or something else, use it as is
                                      parsedResult = invocation.result.content || invocation.result
                                    }
                                  }
                                  
                                  // Filter out frontendScreenshot from parsed result
                                  if (parsedResult && typeof parsedResult === 'object' && 'frontendScreenshot' in parsedResult) {
                                    parsedResult = { ...parsedResult }
                                    delete parsedResult.frontendScreenshot
                                  }
                                  
                                  // Get a quick summary based on tool type
                                  if ((invocation.toolName === 'webSearch' || invocation.toolName === 'googleSearch') && parsedResult) {
                                    const results = Array.isArray(parsedResult) ? parsedResult : []
                                    return results.length > 0 ? `Found ${results.length} results` : 'No results found'
                                  }
                                  if (invocation.toolName === 'imageSearch' && parsedResult) {
                                    const results = Array.isArray(parsedResult) ? parsedResult : []
                                    return results.length > 0 ? `Found ${results.length} images` : 'No images found'
                                  }
                                  if (invocation.toolName === 'codeExecution' && parsedResult) {
                                    const { success } = parsedResult as any
                                    return success ? 'Code ran successfully' : 'Code execution failed'
                                  }
                                  if (invocation.toolName === 'urlScraper' && parsedResult) {
                                    const { title, error } = parsedResult as any
                                    return error ? 'Failed to read page' : title ? `Read "${title.substring(0, 30)}..."` : 'Page scraped'
                                  }
                                  if (invocation.toolName === 'vmScreenshot' && parsedResult) {
                                    const { success, resolution } = parsedResult as any
                                    return success ? `Screenshot captured ${resolution ? resolution : ''}` : 'Screenshot failed'
                                  }
                                  if (invocation.toolName === 'vmAction' && parsedResult) {
                                    const { success, action } = parsedResult as any
                                    return success ? `${action} executed` : `${action} failed`
                                  }
                        ...
                        {expandedTools.has(invocation.id) && (
                          <motion.div 
                            initial={{ height: 0, opacity: 0 }}
                            animate={{ height: "auto", opacity: 1 }}
                            exit={{ height: 0, opacity: 0 }}
                            transition={{ duration: 0.3 }}
                            className="border-t border-border/30 bg-neutral-50/50 dark:bg-neutral-900/50"
                          >
                            {/* Show loading state for in-progress tools */}
                            {(invocation.state === 'call' || invocation.state === 'partial-call') && !invocation.result && (
                              <div className="p-3">
                                <div className="relative">
                                  {/* Sleek monochrome processing indicator */}
                                  <div className="flex items-center gap-3 p-3 rounded-lg bg-gradient-to-r from-zinc-900/50 via-zinc-800/30 to-zinc-900/50 border border-zinc-800/20">
                                    {/* Animated rings */}
                                    <div className="relative">
                                      <motion.div
                                        className="absolute inset-0 rounded-full border border-zinc-600/30"
                                      ...
                                      />
                                      <div className="w-4 h-4 rounded-full bg-zinc-700/50 flex items-center justify-center">
                                        <motion.div
                                          className="w-1.5 h-1.5 rounded-full bg-zinc-300/70"
                                          animate={{ opacity: [0.3, 1, 0.3] }}
                                          transition={{ duration: 1.5, repeat: Infinity }}
                                        />
                                      </div>
                                    ...
                                    <motion.span 
                                      className="text-xs font-light text-zinc-400/90 tracking-wide uppercase"
                                      animate={{ opacity: [0.7, 1, 0.7] }}
                                      transition={{ duration: 2, repeat: Infinity, ease: "easeInOut" }}
                                    >
                                      {invocation.state === 'call' ? 'Initializing' : 'Processing'}
                                    </motion.span>
                                  </div>
                                </div>
                                {invocation.args && (
                                  <div className="bg-neutral-200/30 dark:bg-neutral-900/30 rounded-lg p-3 border border-border/20">
                                    <p className="text-xs text-muted-foreground">
                                      {invocation.toolName === 'googleSearch' && invocation.args.query ? 
                            ...
                            {invocation.result && (
                              <div className="p-3">
                                <div className="flex items-center gap-2 mb-2">
                                  <span className="text-sm font-medium text-muted-foreground">ðŸŽ¯ Results</span>
                                </div>
                                {renderToolResult(invocation)}
                                
                                {/* Display screenshot if available (from frontend capture) - but not for browser tools when already shown */}
                                {invocation.frontendScreenshot && !invocation.toolName.toLowerCase().startsWith('browser') && (() => {
                                  console.log('[ProjectNavigator] Rendering screenshot for', invocation.toolName, 'size:', invocation.frontendScreenshot.length)
                                  return (
                                    <div className="mt-3">
                                      <div className="text-xs text-muted-foreground mb-2">ðŸ“¸ Screenshot after execution:</div>
                                      <div className="relative rounded-lg overflow-hidden border border-border/50 bg-background">
                                        <img
                                          src={invocation.frontendScreenshot}
                                          alt="Screenshot after tool execution"
                                          className="w-full h-auto block" style={{ maxHeight: '75vh', objectFit: 'contain', display: 'block' }}
                                          loading="lazy"
                                          onLoad={() => console.log('[ProjectNavigator] Screenshot loaded successfully')}
                                          onError={(e) => console.error('[ProjectNavigator] Screenshot failed to load:', e)}
                                        />
                                      </div>
                                    </div>
                                  )
