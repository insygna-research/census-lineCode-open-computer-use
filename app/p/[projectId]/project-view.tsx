...
type Attachment = {
  ...
  type: string
  size: number
  url: string  // Required for AI SDK compatibility
  vmPath?: string
}
...
export function ProjectView({ projectId }: ProjectViewProps) {
  ...
  const [enableSearch, setEnableSearch] = useState(true) // Always enable search
  ...
  // File upload functionality removed - not available
  const files: any[] = []
  const setFiles = (files: any) => {}
  const handleFileUploads = (userId: string, chatId: string): Promise<Attachment[]> => Promise.resolve([])
  const createOptimisticAttachments = (files: any) => []
  ...
  // Wrapper function to match useChatOperations signature
  const createNewChatWrapper = useCallback(async (
    userId: string,
    title?: string,
    model?: string,
    ...
    return createNewChat(userId, title, model, isAuthenticated, systemPrompt)
  }, [createNewChat])
  ...
  const { handleDelete, handleEdit } = useChatOperations({
    ...
    chatId: null,
    messages,
    initialMessages: messages, // Use messages as initialMessages for project context
    selectedModel,
    systemPrompt: SystemPrompts.main(),
    createNewChat: createNewChatWrapper,
  ...
  const conversationProps = useMemo(
    ...
    [messages, status, handleEdit, handleReload]
  )

  // Research depth state for project context
