...
import { azureModels } from "./data/azure"
...
const STATIC_MODELS: ModelConfig[] = [
  ...
  ...perplexityModels,
  ...geminiModels,
  // ...ollamaModels, // Static fallback Ollama models - COMMENTED OUT
  ...openrouterModels,
  ...azureModels,
]
...
export async function getAllModels(): Promise<ModelConfig[]> {
  ...
  try {
    ...
    // const detectedOllamaModels = await getOllamaModels() // COMMENTED OUT
    ...
    // dynamicModelsCache = [...staticModelsWithoutOllama, ...detectedOllamaModels] // COMMENTED OUT
    dynamicModelsCache = [...staticModelsWithoutOllama] // Use static models without Ollama
...
export async function getModelsWithAccessFlags(): Promise<ModelConfig[]> {
  ...
  const freeModels = models
    .filter(
      (model) =>
        FREE_MODELS_IDS.includes(model.id) // || model.providerId === "ollama" // COMMENTED OUT
    )
    .map((model) => ({
      ...model,
      accessible: true,
