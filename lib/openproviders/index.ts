...
type AzureOpenAIProviderSettings = Parameters<ReturnType<typeof createAzure>>[1]
...
export function openproviders<T extends SupportedModel>(
  ...
  // COMMENTED OUT - Ollama provider disabled
  // if (provider === "ollama") {
  ...
  if (provider === "azure") {
    // Use env or apiKey for Azure
    const deployment = process.env.AZURE_OPENAI_DEPLOYMENT || undefined
    const endpoint = process.env.AZURE_OPENAI_ENDPOINT || undefined
    const azureApiKey = apiKey || process.env.AZURE_OPENAI_API_KEY
    if (!deployment || !endpoint || !azureApiKey) {
      throw new Error("Missing Azure OpenAI configuration: AZURE_OPENAI_DEPLOYMENT, AZURE_OPENAI_ENDPOINT, or AZURE_OPENAI_API_KEY")
    }
    ...
    return azureProvider(
      deployment,
      settings as AzureOpenAIProviderSettings
    )
  }
